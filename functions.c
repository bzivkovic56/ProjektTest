...
void kreiranjeDatoteke_Igraci(const char* const datIgrac) {
	FILE* pF = fopen(datIgrac, "w");//17
	if (pF == NULL) {
		perror("Kreiranje datoteke igraci.txt");//20
		exit(EXIT_FAILURE);
	}
...
void dodajIgraca(const char* const datIgrac) {
	FILE* pF = fopen(datIgrac, "r+");
	if (pF == NULL) {
		perror("Dodavanje igraca u datoteku igraci.txt");
		exit(EXIT_FAILURE);
	}
    ...
    } while (1); // Ponavljaj sve dok korisnik ne unese ispravan unos
...
void pronadiIgracaPrezime(IGRAC* const poljeIgraca) {
	...
	if (counter == 0) {
		printf("Nema igraca s unesenim prezimenom\n");
		return;
	}
}
